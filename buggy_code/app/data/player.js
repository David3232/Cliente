...
class Player{
    ...
    static fromDatasetJSONObject(playerId,jsonObject){
        ...
        let valueString = jsonObject[constants.market_value];
        let value =  valueString.substring(1,valueString.length-1);
        if (value.charAt(value.length-1 == 'M')){
          player.value = value*1000000;
        } else {
          player.value = value*1000;
        }
